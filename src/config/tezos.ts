...
export const siteUrl: string =
    (import.meta.env.VITE_SITE_URL || "").trim().replace(/\/+$/, "") || "https://wtftez.com";

/** WTF chat worker URL — used by all chat components for API requests */
export const hackchatUrl: string =
    (import.meta.env.VITE_WTFCHAT_URL || import.meta.env.VITE_HACKCHAT_URL || "")
        .trim()
...
export const parentDomainLabel: string =
    (import.meta.env.VITE_PARENT_DOMAIN_LABEL || "wtf").trim().toLowerCase();

export const parentDomain: string =
...
export const chatParentDomains: string[] =
    ((import.meta.env.VITE_CHAT_PARENT_DOMAINS || `${parentDomainLabel},hack`) as string)
        ...
        .filter(Boolean)
        .map((entry) => entry.includes(".") ? entry : `${entry}.${config.tld}`);

export const chatSigningPrefix: string =
    (import.meta.env.VITE_CHAT_SIGNING_PREFIX || `${parentDomain}-chat`).trim();
