...
// ── Bulk fetch all configured parent-domain subdomains with profile data ────────────
...
/** Fetch all subdomains under the configured parent domain with profile data (paginates 50 at a time) */
...
/** Check if a subdomain of the configured parent domain is available */
export async function checkAvailability(label: string): Promise<boolean> {
    const name = `${label}.${parentDomain}`;
...
export async function getSubdomainsByOwner(ownerAddress: string): Promise<SubdomainRecord[]> {
    const data = await gql<{
      ...
      domains(where: { owner: { equalTo: $owner }, name: { endsWith: $parent } }) {
        ...
        { owner: ownerAddress, parent: `.${parentDomain}` },
    );
...
/** Get the first configured parent-domain subdomain owned by an address (if any) */
async function getFirstChatSubdomain(address: string): Promise<string | null> {
 ...
 *   1. Owned configured parent-domain subdomain
...
export async function resolveDisplayName(address: string): Promise<string | null> {
    const [chatDomain, domain, alias] = await Promise.allSettled([
        getFirstChatSubdomain(address),
        resolveAddressToDomain(address),
        resolveAddressToAlias(address),
    ]);
    const best = (r: PromiseSettledResult<string | null>) =>
        r.status === "fulfilled" ? r.value : null;
    return best(chatDomain) ?? best(domain) ?? best(alias);
}
