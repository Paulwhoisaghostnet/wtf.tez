...
import config, { getTedContracts, parentDomain } from "../config/tezos";
...
/**
 * Create a sub-subdomain (e.g. myproject.joe.wtf.tez).
...
export async function submitCreateSubdomain(
    ...
    const parentName = `${parentLabel}.${parentDomain}`;
    const labelHex = labelToHexBytes(childLabel);
    const parentHex = labelToHexBytes(parentName);
