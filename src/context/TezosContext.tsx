...
import config, { chatSigningPrefix, hackchatUrl } from "../config/tezos";
...
const AUTH_STORAGE_KEY = "wtf-tez-auth-session";
...
async function getOrCreateClient(): Promise<DAppClient> {
    ...
    dAppClient = new sdk.DAppClient({ name: "wtf.tez", network: buildNetwork(sdk) });
    return dAppClient;
}
...
async function authenticateWallet(c: DAppClient, addr: string): Promise<AuthSession> {
    ...
    const challenge = `${chatSigningPrefix}:${timestamp}:${nonce}`;
