...
import { buildChatChallengeMessage } from "../domain-config.js";
 ...
 * The signed message format is: `{CHAT_SIGNING_PREFIX}:{timestamp}:{nonce}`.
 *
...
export async function verifyTezosSignature(params: {
  ...
  const { address, publicKey, signature, timestamp, nonce, challengePrefix = "wtf.tez-chat" } = params;
  ...
  const message = buildChatChallengeMessage(timestamp, nonce, challengePrefix);
