...
/**
 * Query TED GraphQL for all configured chat parent domains owned by an address.
 * Returns full domain names (e.g. ["alice.wtf.tez", "skllz.hack.tez"]).
 *
...
export async function getOwnedDomains(
  ...
  const parents = parentDomains?.length ? parentDomains : [`wtf.${tld}`, `hack.${tld}`];

  const results = await Promise.all(
    parents.map((parent) => tedGql<{
      ...
      { owner: address, parent: `.${parent}` },
    )),
  );

  return [...new Set(results.flatMap((data) => data.domains.items.map((d) => d.name)))];
}
