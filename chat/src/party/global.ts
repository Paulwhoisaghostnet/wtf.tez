...
import { getAdminDomains, getAllowedParentDomains } from "../domain-config.js";
import {
  sendJson,
...
export default class GlobalRoom implements Server {
  ...
  private getWorkerUrl(): string {
    return (this.room.env.WORKER_URL as string) ?? "https://wtfchat.rejkt.workers.dev";
  }
  ...
  private getParentDomains(): string[] {
    return getAllowedParentDomains(this.room.env as Record<string, string | undefined>, this.getNetworkTld());
  }
  ...
  private isAdminConn(conn: Connection): boolean {
    ...
    return getAdminDomains(this.getParentDomains()).includes(activeDomain);
  }
  ...
  private async reverifyOwnership(): Promise<void> {
    ...
    for (const [address, domains] of pairs) {
      ...
      try {
        ownedDomains = await getOwnedDomains(address, network, this.getParentDomains());
      } catch (err) {
