...
const ALLOWED_ORIGINS = [
  "https://wtftez.com",
  "https://www.wtftez.com",
  "https://wtf-tez.netlify.app",
  "https://hacktez.com",
...
function getAllowedOrigins(env?: Partial<Env>): string[] {
  const configured = (env?.CHAT_ALLOWED_ORIGINS ?? "")
    .split(",")
    .map((origin) => origin.trim())
    .filter(Boolean);
  return [...new Set([...configured, ...ALLOWED_ORIGINS])];
}

function isAllowedOrigin(origin: string, env?: Partial<Env>): boolean {
  if (getAllowedOrigins(env).includes(origin)) return true;
  // Netlify deploy previews for wtf.tez and the upstream hack.tez network.
  if (/^https:\/\/[a-z0-9-]+--hacktez\.netlify\.app$/.test(origin)) return true;
  if (/^https:\/\/[a-z0-9-]+--wtf-tez\.netlify\.app$/.test(origin)) return true;
  return false;
}

function getCorsHeaders(request: Request, env?: Partial<Env>): Record<string, string> {
  const origin = request.headers.get("Origin") ?? "";
  const allowedOrigins = getAllowedOrigins(env);
  const allowedOrigin = isAllowedOrigin(origin, env) ? origin : allowedOrigins[0];
  return {
...
function corsResponse(
  request: Request,
  envOrBody: Env | string | null,
  bodyOrStatus?: string | null | number,
  maybeStatus = 200,
): Response {
  const hasEnv = typeof envOrBody === "object" && envOrBody !== null && "CHAT_JWT_SECRET" in envOrBody;
  const env = hasEnv ? envOrBody : undefined;
  const body = hasEnv ? (bodyOrStatus as string | null) : (envOrBody as string | null);
  const status = hasEnv
    ? maybeStatus
    : typeof bodyOrStatus === "number" ? bodyOrStatus : 200;
...
function errorResponse(
  ...
  if (typeof envOrError === "string") {
    return corsResponse(
      request,
      JSON.stringify({ error: envOrError, code: errorOrCode }),
      codeOrStatus as number,
    );
  }
  return corsResponse(
    request,
    envOrError,
    JSON.stringify({ error: errorOrCode, code: codeOrStatus }),
    maybeStatus,
  );
}
...
function requireDomain(request: Request, envOrUser: Env | JwtPayload, maybeUser?: JwtPayload): Response | null {
  const env = maybeUser ? envOrUser as Env : undefined;
  const user = maybeUser ?? envOrUser as JwtPayload;
  if (!user.activeDomain) {
    const parents = env ? getAllowedParentDomains(env, getNetworkTld(env)) : ["wtf.tez", "hack.tez"];
    return env
      ? errorResponse(request, env, `A ${parents.join(" or ")} domain is required for this action`, "NO_DOMAIN", 403)
      : errorResponse(request, `A ${parents.join(" or ")} domain is required for this action`, "NO_DOMAIN", 403);
  }
  return null;
}
...
function isAdmin(jwt: JwtPayload, env: Env): boolean {
  const parentDomains = getAllowedParentDomains(env, getNetworkTld(env));
  return getAdminDomains(parentDomains).some((domain) => jwt.domains.includes(domain));
}
...
function normalizeDmTargetDomain(input: string, env: Env): { ok: true; domain: string } | { ok: false; error: string } {
  return normalizeChatDomain(input, getAllowedParentDomains(env, getNetworkTld(env)));
}
...
async function handleAuth(request: Request, env: Env): Promise<Response> {
  ...
  try {
    const parentDomains = getAllowedParentDomains(env, getNetworkTld(env));
    valid = await verifyTezosSignature({
      address,
      publicKey,
      signature,
      timestamp,
      nonce,
      challengePrefix: getChatSigningPrefix(env, parentDomains),
    });
  } catch (err) {
  ...
  let domains: string[];
  try {
    domains = await getOwnedDomains(address, network, parentDomains);
  } catch {
...
async function handleRefresh(request: Request, env: Env): Promise<Response> {
  ...
  let domains: string[];
  try {
    domains = await getOwnedDomains(user.address, network, parentDomains);
  } catch {
...
async function handleDmCreate(request: Request, env: Env): Promise<Response> {
  ...
  const normalized = normalizeDmTargetDomain(targetDomain, env);
  if (!normalized.ok) {
...
async function dispatchPushNotifications(
  ...
  if (roomId.startsWith("dm:")) {
    ...
    const mentioned = detectMentions(content, getAllowedParentDomains(env, tld));
...
async function handleAdminBroadcast(request: Request, env: Env): Promise<Response> {
  ...
  const parentDomains = getAllowedParentDomains(env, getNetworkTld(env));
  const sigValid = await verifyTezosSignature({
...
async function handleGifSearch(request: Request, env: Env): Promise<Response> {
  ...
  klipyUrl.searchParams.set("client_key", "wtfchat");
...
async function handleOgMeta(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    const resp = await fetch(targetUrl, {
      headers: {
        "User-Agent": "wtfchat-link-preview/1.0 (compatible; bot)",
        "Accept": "text/html",
      },
      redirect: "follow",
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (path === "/health") {
      return corsResponse(request, JSON.stringify({ status: "ok", service: "wtfchat" }));
    }
