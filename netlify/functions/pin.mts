 ...
 *   nonce      — random string
 *   signature  — Tezos signature of "wtf.tez — Authorize N image upload(s) · <ISO> · <nonce>"
...
export default async function handler(req: Request, _ctx: Context): Promise<Response> {
    ...
    const message = `wtf.tez — Authorize ${fileCount} image upload${fileCount > 1 ? "s" : ""} · ${date} · ${nonce}`;
    ...
    try {
        const data = await tedGql<{
            ...
            { owner: address, parent: `.wtf.${net.tld}` },
        );
    ...
    if (!ownsDomain) {
        return err("Address does not own a *.wtf.tez domain", "NO_DOMAIN", 401);
    }
