...
function getParentDomain(net: { tld: "tez" | "gho" }): string {
    const configured = (process.env.PARENT_DOMAIN || process.env.VITE_PARENT_DOMAIN || "").trim().toLowerCase();
    if (!configured) return `wtf.${net.tld}`;
    return configured.includes(".") ? configured : `${configured}.${net.tld}`;
}

function normalizeLabel(nameOrLabel: string, net: { tld: "tez" | "gho" }): string {
    const value = nameOrLabel.trim().toLowerCase();
    const suffix = `.${getParentDomain(net)}`;
...
async function handleDomain(name: string, net: ReturnType<typeof getNetwork>): Promise<Response> {
    const parentDomain = getParentDomain(net);
    const label = normalizeLabel(name, net);
...
async function handleAvailability(label: string, net: ReturnType<typeof getNetwork>): Promise<Response> {
    ...
    const fullName = `${label}.${getParentDomain(net)}`;
...
async function handleOwner(address: string, net: ReturnType<typeof getNetwork>): Promise<Response> {
    ...
    const data = await tedGql<{
        ...
        { owner: address, parent: `.${getParentDomain(net)}` },
    );

    const domains = data.domains.items.map((d) => ({
        name: d.name,
        label: d.name.replace(`.${getParentDomain(net)}`, ""),
        address: d.address,
        owner: d.owner,
    }));
...
async function handleResolve(address: string, net: ReturnType<typeof getNetwork>): Promise<Response> {
    ...
    const wtfTezDomains = ownerData.domains.items.map((d) => d.name);
    // TED reverse record if set, else first owned WTF parent-domain record, else null
    const primary = reverseData.reverseRecord?.domain?.name ?? wtfTezDomains[0] ?? null;

    return json(
        {
            address,
            primary,
            wtfTez: wtfTezDomains,
            hackTez: wtfTezDomains,
            network: net.name,
        },
        200,
        { "Cache-Control": "public, s-maxage=60, stale-while-revalidate=120" },
...
/** GET /api/v1/domains?limit=50 — list all configured parent-domain registrations */
async function handleDomains(url: URL, net: ReturnType<typeof getNetwork>): Promise<Response> {
    const parent = getParentDomain(net);
...
async function handleActivity(url: URL, net: ReturnType<typeof getNetwork>): Promise<Response> {
    ...
    const claims = allClaims.map((op) => {
        ...
        return {
            ...
            name: label ? `${label}.${getParentDomain(net)}` : null,
...
async function handleAvatar(label: string, reqUrl: URL, net: ReturnType<typeof getNetwork>): Promise<Response> {
    ...
    const fullName = `${normalizedLabel}.${getParentDomain(net)}`;
